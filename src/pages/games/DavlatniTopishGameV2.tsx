...
export default function DavlatniTopishGameV2() {
  ...
  useEffect(() => {
    fetch("/api/games/questions/davlatni-topish?difficulty=Oson")
      .then((r) => r.json())
    ...
    return (
      <GameProLayout accentColor="cyan">
        <div className="min-h-screen flex items-center justify-center">
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="rounded-full border-2 border-white/20 border-t-cyan-400/80 w-16 h-16" />
      ...
      <GameProLayout accentColor="cyan">
        <div className="min-h-screen flex items-center justify-center p-4">
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 200 }} className="game-pro-card p-12 max-w-md text-center">
            <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 1.5, repeat: Infinity }} className="text-7xl mb-6" style={{ filter: "drop-shadow(0 0 30px rgba(34,211,238,0.4))" }}>🌍</motion.div>
            <h2 className="game-pro-title text-4xl mb-2">O'YIN TUGADI!</h2>
            <p className="text-2xl font-bold text-white mb-8">Jami Ball: <span className="text-cyan-400" style={{ textShadow: "0 0 30px rgba(34,211,238,0.5)" }}>{score}</span></p>
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          <div className="game-pro-card p-4">
            <div className="h-1.5 bg-white/5 rounded-full overflow-hidden mb-3">
              <motion.div className="h-full rounded-full bg-gradient-to-r from-cyan-500 to-blue-500" initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 0.5 }} style={{ boxShadow: "0 0 20px rgba(34,211,238,0.5)" }} />
            </div>
            <div className="flex justify-between items-center">
              <span className="text-white/80 font-semibold flex items-center gap-2"><Globe className="w-5 h-5 text-cyan-400" /> Savol {currentQuestion + 1}/{questions.length}</span>
              <span className="text-xl font-bold text-cyan-400" style={{ textShadow: "0 0 20px rgba(34,211,238,0.4)" }}>{score} <span className="text-white/50 text-sm">ball</span></span>
            </div>
          </div>

          <motion.div key={currentQuestion} initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 200 }} className="flex justify-center">
            <div className="text-[120px] md:text-[150px] p-8 rounded-3xl bg-white/5 border border-white/10" style={{ boxShadow: "0 0 60px -20px rgba(255,255,255,0.2), inset 0 0 60px -30px rgba(34,211,238,0.1)" }}>
              {question.flag || "🌍"}
            </div>
          </motion.div>

          <motion.div key={`card-${currentQuestion}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="game-pro-card p-8">
            <div className="border-l-4 border-cyan-500/60 bg-cyan-500/5 p-6 rounded-r-2xl mb-8">
              ...
              {question.opts.map((opt, idx) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`w-full p-4 rounded-2xl font-semibold text-left transition-all duration-300 border-2 flex items-center gap-3 ${
                      ...
                      !answered && isSelected ? "bg-cyan-500/20 border-cyan-400/60 text-white" :
                      !answered ? "bg-white/5 border-white/10 text-white hover:border-cyan-500/50 hover:bg-white/10" :
                      "bg-white/5 border-white/5 text-white/60"
                    }`}
                    style={showCorrect ? { boxShadow: "0 0 30px rgba(52,211,153,0.3)" } : !answered && isSelected ? { boxShadow: "0 0 20px rgba(34,211,238,0.2)" } : {}}
                  >
            ...
            {answered && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white/5 border border-white/10 p-4 rounded-xl mb-6">
                <p className="text-sm text-white/80"><span className="font-bold text-cyan-400">💡 Tushuntirish:</span> {question.explanation}</p>
              </motion.div>
            )}
